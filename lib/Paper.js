...
class Paper {
  constructor(id, title, abstract, authors, keywords, isAccepted, submittedAt, updatedAt, files, session, positionInSession, raw) {
    ...
    this.exportedProperties = ['id', 'title', 'abstract', 'authors', 'keywords', 'isAccepted', 'submittedAt', 'updatedAt', 'files', 'session', 'positionInSession'];
    this.raw = raw;
  }

  getExportObject() {
    var obj = {};
    for (let key in this.exportedProperties) {
  ...
  toXML() {
    return js2xmlparser.parse('paper', this.getExportObject());
  }
